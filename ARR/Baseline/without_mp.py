...
def attack(model_name, filename, n, k):
                ...
                for line in f:
                        ...
                        for index in range(len(listA)):
                            ...
                            if index >=0 and index < n:   
                                prompt_template_vicuna =  sensibletext + " " + listA[index]
        ...
        if model_name == "llama2-7b":
                ...
                model_nm = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf", device_map="auto")
                model_nm.eval()
                for i, instnce in enumerate(listD):
                    attack = instnce.strip()
                    inputs = tokenizer(attack, return_tensors="pt").to(model_nm.device)
                    for z in range(k):
                            with torch.no_grad():
                               outputs = model_nm.generate(**inputs, max_new_tokens=8192)
                            response_llama27b = tokenizer.decode(outputs[0], skip_special_tokens=True)
                            print("response_llama27b before func = ", i, "  ", z, " ", response_llama27b)
                            ...
                            print("response_llama27b after = ", i, "  ", z, " ", response_llama27b)
...
def main(
    input_file="****/sit_context_removed.jsonl",
    **kwargs
):
    n, k = 5, 3
    models = ["llama2-7b"]
    files = ["adversarial_texts_p_nuclear.out"]
    for filename in files:
            for model in models:
                    ...
                    print("output_file name is = ", output_file)
